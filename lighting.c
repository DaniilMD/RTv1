...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/01/03 16:05:13 by openelop          #+#    #+#             */
/*   Updated: 2020/01/03 16:05:15 by openelop         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		get_lighting_params(t_param *param, t_inter_res *inter_res)
{
	...
	if (ft_strcmp(param->figures[inter_res->closest_figure].type,
		...
		"plane:") == 0)
		param->vec_params.normal = param->figures[inter_res->closest_figure].direction;//vec_substr(&(param->vec_params.p_big), &(param->figures[inter_res->closest_figure].direction));//param->figures[inter_res->closest_figure].direction;//vec_sum(param->vec_params.normal,
			//param->vec_params.p_big);
...
void		compute_lighting(t_param *param, t_inter_res *inter_res)
{
	...
	while (i < param->lights_num)
	{
		...
		{
			if (strcmp(param->lights[i].type, "point") == 0)
